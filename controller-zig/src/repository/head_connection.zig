...
pub fn load(allocator: std.mem.Allocator, io: std.Io, data_dir: []const u8) !?Connection {
    ...
    return .{ .allocator = allocator, .name = owned_name, .url = owned_url, .api_key = try allocator.dupe(u8, api_key) };
}

pub fn save(allocator: std.mem.Allocator, io: std.Io, data_dir: []const u8, name: []const u8, url: []const u8, api_key: []const u8) !void {
    const connection_path = try path(allocator, data_dir);
    defer allocator.free(connection_path);
