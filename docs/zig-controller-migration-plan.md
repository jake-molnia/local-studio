...
- Runtime jobs create and update controller-managed vLLM, SGLang, and MLX Python environments when no configured upgrade command overrides them. The installer serializes each backend across processes with the existing lock path, creates mode-`0700` venv directories, prefers `uv` with a checked pip fallback, supports version pins and bundled vLLM wheels, updates explicit Python targets, bounds command output and time, probes the installed package before success, and refreshes runtime discovery and target update metadata immediately. Live fixture acceptance created a fresh MLX venv, selected exact `vllm==0.12.4` and `sglang[all]==0.5.6` arguments, updated an external SGLang Python, rejected a forged shell target, serialized two MLX updates, transferred the lock after cancellation, and rediscovered the managed MLX target with launch and update capabilities after restart without network access.
- Runtime jobs also provision managed llama.cpp source builds under `<data_dir>/runtime/llamacpp`. Zig checks git and CMake, shallow-clones or fast-forwards the canonical repository, enables CUDA when `nvcc` is runnable, disables curl, tests, and examples, sizes parallel compilation from the host CPU count, builds only `llama-server`, verifies the resulting binary, and retains configured-command precedence. All stages use the same cancellable process-group owner, 45-minute bounds, backend install lock, progress records, and output limits. Live offline fixtures exercised clone, configure, build, version probing, exact managed-path output, and restart discovery with the fixture tool directory removed from `PATH`; the managed binary returned launch and update capabilities.
- Worker and Standalone own `GET /v1/huggingface/models`. Zig preserves the upstream search, filter, sort, pagination, exact-model promotion, case-insensitive deduplication, normalization, and upstream status contract while retaining unknown model metadata. Each request follows at most five redirects, caps the response at 8 MiB, and is cancelled after 15 seconds; network and decode failures return the existing 503 shape. Live loopback acceptance verified encoded filters and slash-delimited model IDs, exact-first pagination, malformed-field normalization, unknown-field retention, 429 propagation, and unavailable-upstream degradation.
- Worker and Standalone initialize Bun's `model_downloads` table and own `GET /studio/downloads` plus individual lookup. Zig schema-validates bounded opaque records while preserving unknown fields, skips malformed rows, restores derived total-byte semantics, and atomically rehydrates queued or downloading work to paused with the existing restart error before serving requests. Live restart acceptance verified newest-update ordering, unknown-field retention, complete-file total repair, incomplete-total nulling, malformed-row isolation, the canonical 404, and SQLite persistence readable by the existing schema.
- Worker and Standalone own download creation, pause, resume, and cancel through a bounded eight-run Zig owner. Hugging Face metadata selection preserves token precedence, revision pinning, default ignores, case-insensitive globs, exact file sizes, completed and resumable record reuse, and the multiple-GGUF guard. Physical target reservations reject parent, child, case-folded, and symlink-alias overlap; every selected file is rechecked against the physical target before creation. Streaming uses mode-`0600` `.part` files, the actual partial-file size, Range/206 append, 200 reset, 416 finalization, 750 ms persistence throttling, atomic rename, strict incomplete-body failure, five bounded manual redirects, and cross-origin authorization stripping while retaining Range. Pause, cancel, and shutdown cancel the active HTTP read without persisting tokens or leaking owners. Live acceptance covered a 12 MiB slow transfer, nested conflict, symlink escape, body/header/environment token precedence, cross-host token isolation, byte-for-byte SHA-256 after pause/resume, 416 completion, terminal cancel, leak-free shutdown, and restart rehydration.
- Head and Standalone own provider configuration CRUD while Worker rejects those Head routes; Worker and Standalone own provider catalog resolution while Head preserves the selected-Worker proxy boundary. Zig preserves unrelated `studio-settings.json` keys through serialized atomic mode-`0600` writes, never serializes API keys in provider views, and fetches enabled authenticated OpenAI-compatible catalogs with eight-way bounded concurrency, ten-second deadlines, 8 MiB response caps, failed-provider omission, normalized context metadata, and cross-origin redirect credential stripping. Live acceptance covered validation, create, update with an omitted credential, 404 lookup, catalog normalization, a failed sibling provider, file key preservation, Head/Worker/Standalone route disposition, and a cross-host redirect that succeeded only when authorization was removed.
- Worker and Standalone own named process launches through `POST /compute/launch`. Zig serializes placement through the compatible stale-PID lock, derives device and port leases from persisted instance records, supports llama.cpp, MLX, vLLM, SGLang, and ExLlamaV3 launch plans, merges verbatim argument overrides, rejects prohibited vLLM and SGLang flags, persists exact process identity, derives state from liveness and health, and isolates stop, cancellation, supervision, recovery, and shutdown by instance name. Head retains the selected-Worker proxy boundary and does not supervise local records. Live acceptance covered two concurrent Apple unified-memory launches on distinct ports, hardened records and logs, exact argv and environment delivery, duplicate rejection, stop isolation, in-flight cancellation, forbidden-flag rejection, controller-crash recovery, and graceful all-instance eviction.
...
The migration is not production-ready. Real vLLM, SGLang, ExLlamaV3, and MLX acceptance, exclusive multi-GPU placement, Docker supervision, inference usage accounting, remaining SQLite table parity, installer and harness integration, and desktop cutover still require live proof.
